...
{
	//* 2 command-line-arguement :  1. To use q1(cmdl_arg = 1) algo or q5(cmdl_arg = 2) algo.
	//* 							2. To show k most happy and compatible couples.
	...
	int choice = utility::stoint(argv[1]);
	if(choice == 1)
		algorithm::make_couples(b, g, couples);
	else if(choice == 2)
		algorithm::modified_make_couples(b, g, couples);

	utility::store_couples(couples);

	algorithm::distribute_gifts(couples);

	int k = utility::stoint(argv[2]);
